...
%new
- (void)classicFolderInitWithFolder:(SBFolder *)folder orientation:(int)orientation viewMap:(SBIconViewMap *)viewMap {
	...
	SBFolderIcon *folderIcon = [[self folder] icon];

	SBIconView *folderIconView = folderIconView = [viewMap mappedIconViewForIcon:folderIcon];
	if (!folderIconView){
		...
		if ([controller respondsToSelector:@selector(homescreenIconViewMap)])
			homescreenMap = [controller homescreenIconViewMap];
		else
			homescreenMap = [%c(SBIconViewMap) homescreenMap];
		folderIconView = [homescreenMap mappedIconViewForIcon:folderIcon];
	}
	[self setFolderIconView:folderIconView];

	BOOL isFlipped = [folderIconView isInDock];

	NSInteger modernStyle = 12;
	if ([[CSClassicFolderSettingsManager sharedInstance] dark]){
		if (kCFCoreFoundationVersionNumber < 1242)
			modernStyle = 13;
		else
			modernStyle = 14;
	}
...
- (CSClassicFolderView *)initWithFolder:(SBFolder *)folder orientation:(int)orientation viewMap:(SBIconViewMap *)viewMap {
	self = %orig;
	[self classicFolderInitWithFolder:folder orientation:orientation viewMap:viewMap];
	return self;
}

- (CSClassicFolderView *)initWithFolder:(SBFolder *)folder orientation:(int)orientation viewMap:(SBIconViewMap *)viewMap context:(id)context {
	self = %orig;
	[self classicFolderInitWithFolder:folder orientation:orientation viewMap:viewMap];
	return self;
}
...
- (CSClassicFolderView *)initWithConfiguration:(SBFolderControllerConfiguration *)configuration {
	self = %orig;
	[self classicFolderInitWithFolder:configuration.folder orientation:configuration.orientation viewMap:nil];
	return self;
}
...
- (NSInteger)getMaximumIconRowsForPages {
	...
	Class folderIconListViewClass = %c(SBFolderIconListView);

	CGFloat maxIconColumns = 0;
	if ([folderIconListViewClass respondsToSelector:@selector(iconColumnsForInterfaceOrientation:)])
		maxIconColumns = [folderIconListViewClass iconColumnsForInterfaceOrientation:orientation];
	else
		maxIconColumns = [[self currentIconListView] iconColumnsForCurrentOrientation];
	CGFloat maxIconRows = 0;
	if ([folderIconListViewClass respondsToSelector:@selector(maxVisibleIconRowsInterfaceOrientation:)])
		maxIconRows = [folderIconListViewClass maxVisibleIconRowsInterfaceOrientation:orientation];
	else
		maxIconRows = [[self currentIconListView] iconRowsForCurrentOrientation];
...
- (NSArray *)getVisibleViewsUnderFolder {
	...
	if (![[self folderIconView] isInDock]){
